...
from licenses.tasks import check_and_send_email, get_clients_with_expiring_licenses
...
class LicenseExpiryTestCase(TestCase):
    ...
    def init_licenses(self) -> None:
        ...
        one_month_from_now = time_now + \
            relativedelta(months=1) - timedelta(hours=1)
    ...
    @patch('licenses.notifications.EmailNotification.send_notification', Mock())
    @freeze_time("2023-02-18T15:00")
    def test_expiry_logic_nonmonday(self) -> None:
        self.init_licenses()
        ...
        result_notifications = check_and_send_email()
        self.assertEqual(EmailNotification.send_notification.call_count, 1)
        self.assertEqual(LicenseNotification.objects.count(), 2)
        self.assertEqual(Notification.objects.count(), 1)
        self.assertEqual(len(result_notifications), 1) # 1 email sent
        ...
        self.assertEqual(LicenseNotification.objects.count(), 2)
        self.assertEqual(Notification.objects.count(), 1)
        self.assertEqual(len(result_notifications), 0) # No email sent
        ...
        result_notifications = check_and_send_email()
        self.assertEqual(EmailNotification.send_notification.call_count, 2)
    ...
    def test_expiry_logic_monday(self) -> None:
        self.init_licenses()
        clients = get_clients_with_expiring_licenses()
        self.assertEqual(clients.count(), 1)
